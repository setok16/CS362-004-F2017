...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case remodel:
            return remodelCard(state, currentPlayer, handPos, choice1, choice2);
...
{
    //added bug: setting drawntreasure initial value to 1 instead of 0
...
int smithyCard(struct gameState *state, int currentPlayer, int handPos)
    //added bug: +4 cards instead of +4
{
    int i;
    for (i = 0; i < 4; i++)
        {
...
int remodelCard(struct gameState *state, int currentPlayer, int handPos, int choice1, int choice2)
{
    int i, j;
    ...
    //added bug: gained card can cost up to 4 Coins more than trashed card
...
{
    //store card we will trash
    int j = state->hand[currentPlayer][choice2]; //added bug: changed choice1->choice2
